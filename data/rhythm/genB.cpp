...
void increasing(){
	...
	fr1(i,1,n){
		ans.pb(gval(-1e9,1e9));
		while(ans.back()==0) ans.back()=gval(-1e9,1e9);
	}
	sort(ans.begin(),ans.end(),[](int &x,int &y){
		return abs(x)<abs(y);
	});
...
void postive(){
	vector <int> ans;
	fr1(i,1,n) ans.pb(gval(0,1e9));
	a.pb(ans);
}
void negative(){
	vector <int> ans;
	fr1(i,1,n) ans.pb(gval(-1e9,0));
	a.pb(ans);
}
void pos_neg_pos(){
	vector <int> ans;
	fr1(i,1,n){
		if(i&1) ans.pb(gval(0,1e9));
...
void samealot(){
	...
	vector <int> ans;
	int lim=rnd()%(n/10)+1;
	fr1(i,1,lim) based.pb(gval(-1e9,1e9));
	fr1(i,1,n) ans.pb(based[rnd()%lim]);
	a.pb(ans);
}
void many0(){
	vector <int> ans;
