...
char C[N];
const int V=2e9+1;
int gval(int l,int r){return (int)(rnd()%(r-l+1))+l;}
...
void pos_neg_pos_same(){
	vector <int> bp,bn;
	vector <int> ans;
	int lim=rnd()%(n/10)+1;
	fr1(i,1,lim) bp.pb(gval(1,1e9));
	fr1(i,1,lim) bn.pb(gval(-1e9,1));
	fr1(i,1,n){
		if(i&1)	ans.pb(bp[rnd()%lim]);
		else ans.pb(bn[rnd()%lim]);
	}
	a.pb(ans);
}
void many0(){
	vector <int> ans;
	int lim=rnd()%(n/4)+1;
	// fr1(i,1,lim) ans.pb(0);
	// fr1(i,lim+1,n) ans.pb(gval(-1e9,1e9));
	int lst=0;
	fr1(i,1,n){
		if(rnd()%n<=lim) ans.pb(0);
		else{
			if(lst==0) ans.pb(gval(-1e9,1e9));
			if(lst<=0) ans.pb(gval(0,1e9));
			else ans.pb(gval(-1e9,0));
		}
		lst=ans.back();
	}
	a.pb(ans);
...
void many0_same(){
	...
	vector <int> ans;
	int lim1=rnd()%(n/10)+1;
	fr1(i,1,lim1) bp.pb(gval(0,1e9));
	fr1(i,1,lim1) bn.pb(gval(-1e9,0));
	int lim2=rnd()%(n/4)+1;
	// fr1(i,1,lim) ans.pb(0);
	// fr1(i,lim+1,n) ans.pb(gval(-1e9,1e9));
	int lst=0;
	fr1(i,1,n){
		if(rnd()%n<=lim2) ans.pb(0);
		else{
			if(lst==0) ans.pb(gval(-1e9,1e9));
			if(lst<=0) ans.pb(bp[rnd()%lim1]);
			else ans.pb(bn[rnd()%lim1]);
		}
		lst=ans.back();
	}
	a.pb(ans);
